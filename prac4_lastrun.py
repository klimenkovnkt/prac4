...
expName = 'prac4'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\User\\Desktop\\Практикум 4\\prac4_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    word_1 = visual.TextStim(win=win, name='word_1',
        text='',
        font='Arial',
        pos=[0,0], height=80.0, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('letter_memo.csv', selection='1'),
    ...
    for thisTrial in trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('memo.started', globalClock.getTime(format='float'))
        image.setImage(letter_memo)
        image_noise.setImage('noise.jpg')
        fix_cross.setText('+')
        # keep track of which components have finished
        memoComponents = [image, image_noise, fix_cross]
        for thisComponent in memoComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine and routineTimer.getTime() < 8.0:
            ...
            # if image_noise is starting this frame...
            if image_noise.status == NOT_STARTED and tThisFlip >= 7.0-frameTolerance:
                # keep track of start time/frame for later
                image_noise.frameNStart = frameN  # exact frame index
                image_noise.tStart = t  # local t and not account for scr refresh
                image_noise.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(image_noise, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'image_noise.started')
                # update status
                image_noise.status = STARTED
                image_noise.setAutoDraw(True)
            
            # if image_noise is active this frame...
            if image_noise.status == STARTED:
                # update params
                pass
            
            # if image_noise is stopping this frame...
            if image_noise.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_noise.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    image_noise.tStop = t  # not accounting for scr refresh
                    image_noise.tStopRefresh = tThisFlipGlobal  # on global time
                    image_noise.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'image_noise.stopped')
                    # update status
                    image_noise.status = FINISHED
                    image_noise.setAutoDraw(False)
            
            # *fix_cross* updates
            
            # if fix_cross is starting this frame...
            if fix_cross.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
        ...
        for thisComponent in memoComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('memo.stopped', globalClock.getTime(format='float'))
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('test_letters.csv'),
    ...
    for thisTrial_2 in trials_2:
        ...
        _key_resp_allKeys = []
        # keep track of which components have finished
        testingComponents = [word_1, word_2, key_resp]
        ...
        while continueRoutine:
            ...
            # if word_1 is starting this frame...
            if word_1.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                word_1.frameNStart = frameN  # exact frame index
                word_1.tStart = t  # local t and not account for scr refresh
                word_1.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(word_1, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'word_1.started')
                # update status
                word_1.status = STARTED
                word_1.setAutoDraw(True)
            
            # if word_1 is active this frame...
            if word_1.status == STARTED:
                # update params
                pass
            
            # *word_2* updates
            
            # if word_2 is starting this frame...
            if word_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'word_2.started')
                # update status
                word_2.status = STARTED
                word_2.setAutoDraw(True)
            
            # if word_2 is active this frame...
